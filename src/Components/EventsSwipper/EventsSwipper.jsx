      ...
      <Slider {...settings}  ref={slider => {
        ...
        {events?.map((event)=>(
         <div className='min-h-screen relative after:absolute after:z-10 after:content[""] after:top-0 after:left-0 after:w-full after:h-full after:bg-[#373737c1]'>
           <img src={event?.imagePath} className='w-full z-50 object-cover h-full' alt="" />
           <div className="info max-w-[1200px] z-50 my-2 absolute top-[200px] left-[170px]">
              <h1 className='text-white text-[40px] font-[800]'>{event?.title}</h1>
              <p className='max-w-[600px] text-white mt-5'>{event?.description?.substring(0,500)}</p>
              <Btn className='text-white' to={`/events/${event?.id}/${event?.cityId}/${event?.programId}`}>
                read more
              </Btn>
           </div>
        </div>
